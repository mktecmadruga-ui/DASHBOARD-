...
export default function AudienceAnalytics() {
  ...
  return (
    <Card
      delay={0.5}
      title="Audiência"
      subtitle="Demografia, idade, localização"
      ...
      {loading ? (
        <div className="space-y-3" aria-busy="true" aria-live="polite">
          {[120, 80, 100, 60].map((h, i) => (
            <div key={i} className="bg-slate-100 animate-pulse rounded-xl" style={{ height: h }} aria-hidden="true"/>
          ))}
        </div>
      ) : !data ? (
        <p className="text-sm text-text-light text-center py-8" role="status">Sem dados de audiência disponíveis.</p>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              <p className="text-[10px] uppercase tracking-wider font-semibold text-text-light text-center mb-1.5">Gênero</p>
              <DonutChart data={genderChartData} colors={GENDER_COLORS} size={130} />
              <div className="flex flex-col gap-1.5 mt-2">
                {genderChartData.map((g, i) => (
                  <div key={g.label} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full" style={{ background: GENDER_COLORS[i] }} aria-hidden="true"/>
                      <span className="text-xs text-text-medium">{g.label}</span>
                    </div>
                    <span className="text-xs font-semibold text-text-dark tabular-nums">{g.valor}%</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider font-semibold text-text-light text-center mb-1.5">Idade</p>
              <div className="flex flex-col gap-2 mt-2">
                {ageChartData.map((a, i) => (
                  <div key={a.faixa} className="flex items-center gap-2">
                    <span className="text-[11px] text-text-light w-11 tabular-nums">{a.faixa}</span>
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${a.valor}%`, background: AGE_COLORS[i % AGE_COLORS.length] }} />
                    </div>
                    <span className="text-xs font-medium text-text-dark w-8 text-right tabular-nums">{a.valor}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mb-4">
            <p className="text-[10px] uppercase tracking-wider font-semibold text-text-light mb-2.5">Top Cidades</p>
            <div className="flex flex-col gap-2">
              {cities.map((c, i) => (
                <div key={c.name} className="flex items-center gap-3">
                  <span className="text-[11px] text-text-light w-4 font-semibold tabular-nums">{i + 1}</span>
                  <span className="text-xs text-text-medium flex-1 truncate">{c.name}</span>
                  <div className="w-20 h-1.5 bg-slate-100 rounded-full flex-shrink-0 overflow-hidden">
                    <div className="h-full rounded-full gradient-primary transition-all duration-500" style={{ width: `${(c.value / maxCity) * 100}%` }} />
                  </div>
                  <span className="text-xs font-semibold text-text-dark w-8 text-right tabular-nums">{c.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {countries.length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-wider font-semibold text-text-light mb-2.5">Top Países</p>
              <div className="flex flex-wrap gap-1.5">
                {countries.map(c => (
                  <span key={c.code} className="text-[10px] px-2 py-1 rounded-lg bg-slate-50 border border-slate-100 text-text-medium">
                    {COUNTRY_LABELS[c.code] ?? c.code} · <span className="tabular-nums">{c.value}</span>
                  </span>
                ))}
              </div>
            </div>
